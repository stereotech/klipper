...
class WizardStepJog(WizardStep):
    def __init__(self, config):
        ...
        # get options
        self.axes = config.getlists('axes')
        self.steps = config.getfloatlist('steps')
        self.default_step = config.getfloat('default_step')
        # create template
    ...
    def cmd_WIZARD_STEP_SET_STEP(self, gcmd):
        ...
        if value not in self.steps:
            raise gcmd.error(
                "error setting the value:%s to move the axis, the value is out of range" % (value,))
    ...
    def cmd_WIZARD_STEP_JOG(self, gcmd):
        ...
        if axis not in self.axes:
            raise gcmd.error(
                "error moved the axis:%s, the axis not availability" % (axis,))
        if self.in_script:
