...
class ledFrameHandler:
    ...
    def _getFrames(self, eventtime):
        ...
        frames = [(effect, effect.getFrame(eventtime))
                  for effect in self.effects]
...
class ledEffect:
    def __init__(self, config):
        ...
        self.endstops = [x.strip()
                         for x in config.get('endstops', '').split(',')]
    ...
    def _handle_ready(self):
        ...
        for layer in [line for line
            ...
            def pad(x): return x + [0.0] * (COLORS - len(x))
            def convert(s): return float(s)

            try:
                # join all elements of the list
                palette = "".join(parms[4:])
                # remove whitespaces
                palette = "".join(palette.split())
                palette = palette.strip(",")
                # split colors
                palette = palette.split("),(")
                # split color components
                palette = [c.split(",") for c in palette]
                palette = [[convert(k.strip("()")) for k in c]
                           for c in palette]  # convert to float
                for i in palette:
                    if len(i) > COLORS:
                        raise Exception(
                            "Color %s has too many elements." % (str(i),))
                # pad to COLORS colors
                palette = [pad(c) for c in palette]
                # flatten list
                palette = [k for c in palette for k in c]
            except Exception as e:
            ...
            if eventtime >= self.nextEventTime:
                ...
                for layer in self.layers:
                    ...
                    if layerFrame:
                        ...
                        self.frame = [blend(t, b)
                                      for t, b in zip(layerFrame, self.frame)]
        ...
        def _gradient(self, palette, steps, reverse=False, toFirst=False):
            ...
            for i in range(1, steps):
                ...
                if ((k+1) >= len(palette)) | (r == 0.0):
                    ...
                    z = [((1-r)*palette[k][m] + r*palette[k+1][m])
            ...
            if int(self.effectRate/self.frameRate) == 0:
                ...
                for _ in range(len(self.paletteColors) * (self.ledCount-1)):
                    for _ in range(int(self.effectRate/self.frameRate)):
                        self.thisFrame.append(colorArray(COLORS, frame)[
                                              :COLORS*self.ledCount])
    ...
    class layerTemperature(_layerBase):
        def __init__(self,  **kwargs):
            ...
            if len(self.paletteColors) == 1:
                self.paletteColors = colorArray(
                    COLORS, ([0.0]*COLORS)) + self.paletteColors
            gradient = colorArray(
    ...
    class layerStepperColor(_layerBase):
        ...
        def nextFrame(self, eventtime):
            ...
            p = self.frameHandler.stepperPositions[int(
                axis)]*self.effectRate+self.effectCutoff
    ...
    class layerProgress(_layerBase):
        def __init__(self,  **kwargs):
            ...
            self.thisFrame.append(colorArray(
        ...
        def nextFrame(self, eventtime):
            for endstop in self.handler.endstops:
                ...
                if self.my_flag[endstop] != self.frameHandler.homing_end_flag[endstop]:
                    self.counter = 0
                    self.coloridx = (
                        self.coloridx + 1) % len(self.paletteColors)
                    self.my_flag[endstop] = self.frameHandler.homing_end_flag[endstop]

            frame = [self.decayTable[self.counter] *
                     i for i in self.thisFrame[self.coloridx]]
