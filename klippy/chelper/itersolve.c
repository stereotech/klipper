...
static int32_t
itersolve_gen_steps_range(struct stepper_kinematics *sk, struct move *m, double abs_start, double abs_end)
{
    ...
    for (;;)
    {
        ...
        double next_time = ((old_guess.time * guess_dist - guess.time * og_dist) / (guess_dist - og_dist));
...
itersolve_generate_steps(struct stepper_kinematics *sk, double flush_time)
{
    ...
    for (;;)
    {
        ...
        if (check_active(sk, m))
        {
            ...
            int32_t ret = itersolve_gen_steps_range(sk, m, last_flush_time, flush_time);
            if (ret)
                return ret;
        ...
        else
        {
            if (move_start < force_steps_time)
            {
                ...
                if (abs_end > flush_time)
                    abs_end = flush_time;
                int32_t ret = itersolve_gen_steps_range(sk, m, last_flush_time, abs_end);
                if (ret)
                    return ret;
                skip_count = 1;
            }
...
void __visible
itersolve_set_stepcompress(struct stepper_kinematics *sk, struct stepcompress *sc, double step_dist)
{
    sk->sc = sc;
...
void __visible
itersolve_set_position(struct stepper_kinematics *sk, double x, double y, double z, double a, double c)
{
    sk->commanded_pos = itersolve_calc_position_from_coord(sk, x, y, z, a, c);
}
