...
Coord = collections.namedtuple('Coord', ('x', 'y', 'z', 'a', 'c', 'e'))
...
class GCodeDispatch:
    ...
    def respond_warning(self, msg):
        logging.warning(msg)
        self.respond_info(msg, log=False)
    ...
    def _get_extended_params(self, gcmd):
        ...
        if m is None:
            raise self.error("902: Malformed command '%s'"
    ...
    def _cmd_mux(self, command, gcmd):
        ...
        if key_param not in values:
            raise gcmd.error("905: The value '%s' is not valid for %s"
                             % (key_param, key))
